...
const AppStoreView: React.FC<AppStoreViewProps> = ({ currentPlan, onUpgrade }) => {
  const handleUpgrade = (planId: string) => {
    if (planId === currentPlan) {
      alert(`ℹ️ Current Plan\n\nYou are already on the ${planId.toUpperCase()} plan.`);
      return;
    }
    
    // Call the parent upgrade function
    onUpgrade(planId);
    
    alert(`✓ PLAN UPGRADE INITIATED\n\n` +
      `Target Plan: ${planId.toUpperCase()}\n` +
      `Tier: ${planId === 'free' ? 'Standard' : planId === 'business' ? 'Business' : 'Enterprise'}\n\n` +
      `In production, this would:\n` +
      `1. Process payment (if needed)\n` +
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="pt-12 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div 
            onClick={() => {
              const modules = APP_MODULES.slice(8).map(m => `• ${m.name} (${m.category})`).join('\n');
              alert(`📦 COMPLETE MODULE REGISTRY\n\n` +
                `Showing 8+ additional modules:\n\n${modules}\n\n` +
                `All modules are indexed in the global repository.\n` +
                `Access based on your current plan tier.`);
            }}
